...
export class GameService {
  ...
  findGame(socket, data, server, roomData) {
    let currentRoom = [...roomData.entries()]
      .filter(({ 1: v }) => v.status2 == 'created' && v.type == data.type)
      .map(([k]) => k);
    ...
    if (!currentRoom[0]) {
      ...
      roomData.set(currentRoom[0], {
        ...roomData.get(currentRoom[0]),
        status2: 'pending',
        player2: {
          socketId: socket.id,
          score: 0,
      ...
      socket.join(currentRoom[0]);
      server.to(currentRoom[0]).emit('joinRoom', {
        status: 'pending',
        roomName: currentRoom[0],
        player1: roomData.get(currentRoom[0]).player1.socketId,
        player2: roomData.get(currentRoom[0]).player2.socketId,
        type: roomData.get(currentRoom[0]).type,
      });
    }
    // let exist = 0;
